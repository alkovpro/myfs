...
param = lambda x,y,z: x[y] if y in x else z
...
class fsNode(object):
    ...
    def load(self):
        ...
        if self.id is not None:
            ...
            if self.parent is None:
                ...
                query = u'MATCH (f%s{parent:{PARENT},name:{NAME}}) RETURN f' % (':%s' % self.nodetype) if len(self.nodetype)>0 else ''
    ...
    def save(self):
        ...
        if self.id is not None:
            ...
            query = u'MATCH (p:Folder{id:{ID}}) CREATE (f:%s)-[r:`PARENT`]->(p) SET f.id=id(f),%s RETURN id(f) as id' % (self.nodetype,params)
    ...
    def remove(self,force):
        query = u'MATCH (f{id:{ID}}) OPTIONAL MATCH (f)<-[*]-(f1:Folder) OPTIONAL MATCH (f)<-[*]-(f2:File) RETURN [f1.id,f2.id] as children'
        children=[]
...
class folderNode(fsNode):
    ...
    def __init__(self, **kwargs):
        ...
        if root or name=='/':
            ...
            if len(matches)>0:
                ...
                query = u'CREATE (f:Folder{root:true,name:{NAME},created:{CREATED},modified:{CREATED}}) SET f.id=id(f) RETURN id(f) as id'
    ...
    def list(self):
        result = []
        query = u'MATCH (f)-[r:`PARENT`]->(Folder{id:{ID}}) RETURN id(f) as id,f.name,labels(f)[0] AS nodetype ORDER BY nodetype DESC'
        matches=graph.cypher.execute(query,ID=self.id)
        for match in matches:
            params={
                'id' : getattr(match, 'id'),
...
class myfs(object):
    ...
    def get_nodes_for_path(path=''):
        ...
        if path not in ['','/']:
            crumbs = path.split('/')
            if crumbs[0]=='': crumbs = crumbs[1:]    # leading slash
            if crumbs[-1]=='': crumbs = crumbs[:-1]  # ending slash
            for crumb in crumbs:
                counter += 1
                params['cr%i' % counter] = crumb
                query += u'OPTIONAL MATCH (f%i)<-[r%i:`PARENT`]-(f%i{name:{cr%i}}) ' % (counter-1,counter-1,counter,counter)
                query_end.append({
                    'id':'f%i.id' % counter,
                ...
                name = crumb
        if len(query_end)>2: query_end = query_end[-2:]
